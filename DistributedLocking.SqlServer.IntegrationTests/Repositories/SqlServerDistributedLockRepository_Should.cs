...
namespace DistributedLocking.SqlServer.IntegrationTests.Repositories
{
    ...
    public class SqlServerDistributedLockRepository_Should
    {
        ...
        {
            var success = await DistributedLockRepository.TryInsertIfIdentifierNotExistsAsync(
                identifier,
                lockId,
                timeToLive,
            ...
            DistributedLockTimeToLive timeToLive)
        {
            ...
            var tryAcquireTasks = Enumerable.Range(0, 1000)
                .Select(i =>
                    DistributedLockRepository.TryInsertIfIdentifierNotExistsAsync(identifier, lockId, timeToLive, CancellationToken.None))
                .ToList();
